    ...
    public void map(View view) {
        Rx2AndroidNetworking.get("https://fierce-cove-29863.herokuapp.com/getAnUser/{userId}")
                ...
                .map(new Function<ApiUser, String>() {
                    @Override
                    public String apply(ApiUser apiUser) throws Exception {
                        // here we get ApiUser from server
                        User user = new User(apiUser);
                        // then by converting, we are returning user
...
//                });
    ...
    public void fromUserToUserDetail() {
        getUserListObservable()
...
//                        return Observable.just(user);
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .map(new Function<User, UserDetail>() {
                    @Override
                    public UserDetail apply(@NonNull User user) throws Exception {
...
//                        do anything with userDetail
    ...
    private UserDetail getUserDetail(long id) {
        return new UserDetail(id, "Ali:"+ id, "Baig:"+id );
    }
